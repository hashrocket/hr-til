...
Given(/^channel "([^"]*)" has (\d+) more posts than the pagination breakpoint$/) do |name, number|
  breakpoint = ENV.fetch('pagination_breakpoint').to_i
  FactoryGirl.create_list(:post, breakpoint + number.to_i,
    channel: FactoryGirl.create(:channel, name: name)
  )
end

When(/^I visit the channel page for "([^"]*)"$/) do |name|
...
Given(/^author "([^"]*)" has (\d+) more posts than the pagination breakpoint$/) do |name, number|
  breakpoint = ENV.fetch('pagination_breakpoint').to_i
  FactoryGirl.create_list(:post, breakpoint + number.to_i,
    developer: FactoryGirl.create(:developer, username: name)
  )
end

When(/^I visit the author page for "([^"]*)"$/) do |name|
  author = Developer.find_by(username: name)
  visit developer_path(author)
end

Then 'I see posts matching the pagination breakpoint' do
  expect(page).to have_selector('.post__content', count: ENV.fetch('pagination_breakpoint').to_i)
end

Given(/^(\d+) pages' worth of paginated posts exist$/) do |num|
  FactoryGirl.create_list(:post, ENV.fetch('pagination_breakpoint').to_i * num.to_i)
end
